...
class FeatureSelector(Featurizer):
    ...
    def fit(self, traj_list, y=None):
        """
        Calls fit on all the feature extractors.  This is useful if fit is required for example
        those features that use data from all trajectories, e.g. HBondFeaturizer.
        :param traj_list:
        :param y:
        :return: self
        """
        for feat in self.which_feat:
            self.features[feat].fit(traj_list)
        return self
